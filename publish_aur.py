...
def check_aur_tools():
    ...
    if not result:
        print("❌ makepkg not found. Please install base-devel on Arch Linux")
        return False
    
    # Check if aurpublish is available
    result = run_command("aurpublish --version", "Checking aurpublish")
    if not result:
        print("❌ aurpublish not found. Please install aurpublish")
...
def main():
    ...
    if not check_aur_tools():
        print("❌ AUR tools not available. Exiting.")
        sys.exit(1)
